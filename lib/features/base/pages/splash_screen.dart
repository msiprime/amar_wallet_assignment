...
import 'package:amar_wallet_assignment/features/home/presentation/view/home_page.dart';
...
class _SplashScreenState extends State<SplashScreen> {
  ...
  Widget build(BuildContext context) {
    return BlocListener<AppMetaDataCubit, AppMetaDataState>(
      listener: (context, appMetaDataState) {
        if (appMetaDataState is AppMetaDataLoaded) {
          Navigator.of(context).pushReplacement(
            MaterialPageRoute(builder: (_) => HomePage()),
          );
        }
        if (appMetaDataState is AppMetaDataLoadingFailed) {
          Navigator.of(context).pushReplacement(
            MaterialPageRoute(
              builder: (_) => Scaffold(
                body: Center(
                  child: Text(
                    'Failed to load app metadata. Please try again later.',
